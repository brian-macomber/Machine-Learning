...
for k = 1 : num_cluster
    theta = 2 * pi * rand(points_per_cluster, 1);
    rho = k + randn(points_per_cluster, 1) * bandwidth;
    [x, y] = pol2cart(theta, rho);
    data(idx:idx+points_per_cluster - 1,:) = [x, y];
    label(idx:idx+points_per_cluster - 1)=k;
    idx = idx + points_per_cluster;
end
